...
GORM_Hash* GORM_Hash::m_pInstance = new GORM_Hash();

GORM_Hash* GORM_Hash::Instance()
{
    return m_pInstance;
}
...
uint32 GORM_Hash::Crc16(const char * szKey, size_t iKeyLen)
{
    uint64 x;
    uint32 crc = 0;
...
uint32
GORM_Hash::Crc32(const char *szKey, size_t iKeyLen)
{
    uint64 x;
...
uint32
GORM_Hash::Crc32_1(const char *szKey, size_t iKeyLen)
{
    uint64 x;
    uint32 crc = UINT32_MAX;
...
uint32 GORM_Hash::Crc32_2(const char *szKey, size_t iKeyLen, const char *szKey1, size_t iKeyLen1)
{
    uint64 x;
    uint32 crc = UINT32_MAX;
    ...
    for (x = 0; x < iKeyLen1; x++) {
        crc = (crc >> 8) ^ crc32tab[(crc ^ (uint64)szKey1[x]) & 0xff];
    }

    return ((~crc) >> 16) & 0x7fff;
}
uint32 GORM_Hash::Crc32_3(const char *szKey, size_t iKeyLen, const char *szKey1, size_t iKeyLen1, const char *szKey2, size_t iKeyLen2)
{
    uint64 x;
    uint32 crc = UINT32_MAX;
    ...
    for (x = 0; x < iKeyLen2; x++) {
        crc = (crc >> 8) ^ crc32tab[(crc ^ (uint64)szKey2[x]) & 0xff];
    }

    return ((~crc) >> 16) & 0x7fff;
}
uint32 GORM_Hash::Crc32_4(const char *szKey, size_t iKeyLen, const char *szKey1, size_t iKeyLen1, const char *szKey2, size_t iKeyLen2, const char *szKey3, size_t iKeyLen3)
{
    uint64 x;
    uint32 crc = UINT32_MAX;
    ...
    for (x = 0; x < iKeyLen3; x++) {
        crc = (crc >> 8) ^ crc32tab[(crc ^ (uint64)szKey3[x]) & 0xff];
    }
...
uint32
GORM_Hash::Crc32a(const char *key, size_t key_length)
{
    const char *p = key;
    uint32 crc;
...
uint32
GORM_Hash::Murmur(const char *szKey, size_t iKeyLen)
{
...
uint32 GORM_Hash::Md5(const char *szKey, size_t iKeyLen)
{
    
    unsigned char results[16];
    GORM_Hash::Md5Signature((unsigned char*)szKey, (unsigned long)iKeyLen, results);
...
void GORM_Hash::Md5Signature(unsigned char *key, unsigned long length, unsigned char *result)
{
    MD5_CTX my_md5;
...
uint32
GORM_Hash::Fnv164(const char *key, size_t key_length)
{
...
uint32
GORM_Hash::Fnv1a64(const char *key, size_t key_length)
{
...
uint32
GORM_Hash::Fnv132(const char *key, size_t key_length)
{
...
uint32
GORM_Hash::Fnv1a32(const char *key, size_t key_length)
{
    uint32 hash = FNV_32_INIT;
    size_t x;
...
uint32 
GORM_Hash::Hsieh(const char *szKey, size_t iKeyLen)
{
    uint32_t hash = 0, tmp;
    int rem;
...
uint32 
GORM_Hash::Jenkins(const char *szKey, size_t iKeyLen)
{  
