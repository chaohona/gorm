...
#ifdef GORM_DEBUG
        GORM_MySQLUpdateTableSchema(pMySQLEvent, "login_log", table.custom_columns());
#endif
...
#ifdef GORM_DEBUG
        GORM_MySQLUpdateTableSchema(pMySQLEvent, "res_log", table.custom_columns());
#endif
...
#define ROLEINSERTSQL "insert into role_%d(`version`, `roleid`, `rolename`, `level`, `exp`, `characterid`, `gold`, `offlinetime`, `inited`, `createtime`, `face`, `online`, `worldid`, `pttype`, `userid`) values (%llu, %lld, '%s', %d, %lld, %d, %d, %lld, %d, %lld, '%s', %lld, %lld, %d, %lld);"
int GORM_PackInsertSQLROLE_ONE(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, MYSQL* mysql, int iTableIndex, const GORM_PB_Table_role &table_role, GORM_MemPoolData *&pReqData)
{
    ...
    int iLen = iSqlLen + 128 + 8 + 8 + len_role_rolename + 8 + 8 + 8 + 8 + 8 + 8 + 8 + len_role_face + 8 + 8 + 8 + 8;
    GORM_MallocFromSharedPool(pMemPool, pReqData, iLen);
    szSQLBegin = pReqData->m_uszData;
    iLen = GORM_SafeSnprintf(szSQLBegin, iLen, ROLEINSERTSQL, iTableIndex, role_version, role_roleid, sz_role_rolename, role_level, role_exp, role_characterid, role_gold, role_offlinetime, role_inited, role_createtime, sz_role_face, role_online, role_worldid, role_pttype, role_userid);
    pReqData->m_sUsedSize = iLen;
...
#ifdef GORM_DEBUG
        GORM_MySQLUpdateTableSchema(pMySQLEvent, "ptuser", table.custom_columns());
#endif
...
int GORM_PackGetSQLEQUIP_ONE_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_Table_equip &table_equip, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
    GORM_MySQLUpdateTableSchema(pMySQLEvent, "equip", table.custom_columns());
    return GORM_PackGetSQLEQUIP_ONE_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table_equip, pReqData);
#endif
...
int GORM_PackGetSQLFOOD_ONE_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_Table_food &table_food, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
    GORM_MySQLUpdateTableSchema(pMySQLEvent, "food", table.custom_columns());
    return GORM_PackGetSQLFOOD_ONE_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table_food, pReqData);
#endif
...
int GORM_PackGetSQLHERO_ONE_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_Table_hero &table_hero, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
    GORM_MySQLUpdateTableSchema(pMySQLEvent, "hero", table.custom_columns());
    return GORM_PackGetSQLHERO_ONE_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table_hero, pReqData);
#endif
...
#define LOGIN_LOGGETSQL_DEBUG_WHERE " from login_log_%d where  `roleid`=%lld;"
int GORM_PackGetSQLLOGIN_LOG_ONE_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_Table_login_log &table_login_log, GORM_MemPoolData *&pReqData)
{
    ...
    int iSqlLen = 93;
    int iTmpLen = 0;
    auto itr = pMySQLEvent->m_mapTablesColumnInfo.find(string("login_log"));
    if (itr == pMySQLEvent->m_mapTablesColumnInfo.end())
    {
        GORM_LOGE("login_log table info not inited");
        return GORM_ERROR;
    }
    unordered_map<string, GORM_PB_COLUMN_TYPE> &columnMap = itr->second;
    if (columnMap.size() == 0)
        return GORM_ERROR;
    int iTotalLen = iSqlLen + 128 + 64*columnMap.size() + table_login_log.ByteSizeLong();
    int iLen = 7;
    GORM_MallocFromSharedPool(pMemPool, pReqData, iTotalLen);
    szSQLBegin = pReqData->m_uszData;
    memcpy(szSQLBegin, "select ", 7);
    int idx = 0;
    vector<string> &vectorColumns = pMySQLEvent->m_mapTablesColumnOrder["login_log"];
    if (vectorColumns.size() != columnMap.size())
        return GORM_ERROR;
    for (auto c : vectorColumns)
    {
    ...
    const int64 login_log_roleid = table_login_log.roleid();
    iLen += GORM_SafeSnprintf(szSQLBegin+iLen, iTotalLen-iLen, LOGIN_LOGGETSQL_DEBUG_WHERE, iTableIndex, login_log_roleid);
    pReqData->m_sUsedSize = iLen;
...
#ifdef GORM_DEBUG
    GORM_MySQLUpdateTableSchema(pMySQLEvent, "login_log", table.custom_columns());
    return GORM_PackGetSQLLOGIN_LOG_ONE_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table_login_log, pReqData);
#endif
...
int GORM_PackGetSQLMAIL_ONE_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_Table_mail &table_mail, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
    GORM_MySQLUpdateTableSchema(pMySQLEvent, "mail", table.custom_columns());
    return GORM_PackGetSQLMAIL_ONE_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table_mail, pReqData);
#endif
...
int GORM_PackGetSQLMATERIAL_ONE_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_Table_material &table_material, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
    GORM_MySQLUpdateTableSchema(pMySQLEvent, "material", table.custom_columns());
    return GORM_PackGetSQLMATERIAL_ONE_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table_material, pReqData);
#endif
...
int GORM_PackGetSQLNPC_ONE_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_Table_npc &table_npc, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
    GORM_MySQLUpdateTableSchema(pMySQLEvent, "npc", table.custom_columns());
    return GORM_PackGetSQLNPC_ONE_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table_npc, pReqData);
#endif
...
#define RES_LOGGETSQL_DEBUG_WHERE " from res_log_%d where  `roleid`=%lld;"
int GORM_PackGetSQLRES_LOG_ONE_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_Table_res_log &table_res_log, GORM_MemPoolData *&pReqData)
{
    ...
    int iSqlLen = 93;
    int iTmpLen = 0;
    auto itr = pMySQLEvent->m_mapTablesColumnInfo.find(string("res_log"));
    if (itr == pMySQLEvent->m_mapTablesColumnInfo.end())
    {
        GORM_LOGE("res_log table info not inited");
        return GORM_ERROR;
    }
    unordered_map<string, GORM_PB_COLUMN_TYPE> &columnMap = itr->second;
    if (columnMap.size() == 0)
        return GORM_ERROR;
    int iTotalLen = iSqlLen + 128 + 64*columnMap.size() + table_res_log.ByteSizeLong();
    int iLen = 7;
    GORM_MallocFromSharedPool(pMemPool, pReqData, iTotalLen);
    szSQLBegin = pReqData->m_uszData;
    memcpy(szSQLBegin, "select ", 7);
    int idx = 0;
    vector<string> &vectorColumns = pMySQLEvent->m_mapTablesColumnOrder["res_log"];
    if (vectorColumns.size() != columnMap.size())
        return GORM_ERROR;
    for (auto c : vectorColumns)
    {
    ...
    const int64 res_log_roleid = table_res_log.roleid();
    iLen += GORM_SafeSnprintf(szSQLBegin+iLen, iTotalLen-iLen, RES_LOGGETSQL_DEBUG_WHERE, iTableIndex, res_log_roleid);
    pReqData->m_sUsedSize = iLen;
...
#ifdef GORM_DEBUG
    GORM_MySQLUpdateTableSchema(pMySQLEvent, "res_log", table.custom_columns());
    return GORM_PackGetSQLRES_LOG_ONE_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table_res_log, pReqData);
#endif
...
int GORM_PackGetSQLINTERACTION_ONE_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_Table_interaction &table_interaction, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
    GORM_MySQLUpdateTableSchema(pMySQLEvent, "interaction", table.custom_columns());
    return GORM_PackGetSQLINTERACTION_ONE_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table_interaction, pReqData);
#endif
...
int GORM_PackGetSQLROLES_ONE_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_Table_roles &table_roles, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
    GORM_MySQLUpdateTableSchema(pMySQLEvent, "roles", table.custom_columns());
    return GORM_PackGetSQLROLES_ONE_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table_roles, pReqData);
#endif
    return GORM_PackGetSQLROLES_ONE(pMemPool, mysql, iTableIndex, table_roles, pReqData);
}
#define ROLEGETSQL "select `version`, `roleid`, `rolename`, `level`, `exp`, `characterid`, `gold`, `offlinetime`, `inited`, `createtime`, `face`, `online`, `worldid`, `pttype`, `userid` from role_%d where  `roleid`=%lld;"
...
int GORM_PackGetSQLROLE_ONE_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_Table_role &table_role, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
    GORM_MySQLUpdateTableSchema(pMySQLEvent, "role", table.custom_columns());
    return GORM_PackGetSQLROLE_ONE_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table_role, pReqData);
#endif
...
int GORM_PackGetSQLSCENE_ONE_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_Table_scene &table_scene, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
    GORM_MySQLUpdateTableSchema(pMySQLEvent, "scene", table.custom_columns());
    return GORM_PackGetSQLSCENE_ONE_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table_scene, pReqData);
#endif
...
int GORM_PackGetSQLSKILL_ONE_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_Table_skill &table_skill, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
    GORM_MySQLUpdateTableSchema(pMySQLEvent, "skill", table.custom_columns());
    return GORM_PackGetSQLSKILL_ONE_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table_skill, pReqData);
#endif
...
#define PTUSERGETSQL_DEBUG_WHERE " from ptuser_%d where  `ptid`='%s' and `pttype`=%d;"
int GORM_PackGetSQLPTUSER_ONE_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_Table_ptuser &table_ptuser, GORM_MemPoolData *&pReqData)
{
    ...
    int iSqlLen = 93;
    int iTmpLen = 0;
    auto itr = pMySQLEvent->m_mapTablesColumnInfo.find(string("ptuser"));
    if (itr == pMySQLEvent->m_mapTablesColumnInfo.end())
    {
        GORM_LOGE("ptuser table info not inited");
        return GORM_ERROR;
    }
    unordered_map<string, GORM_PB_COLUMN_TYPE> &columnMap = itr->second;
    if (columnMap.size() == 0)
        return GORM_ERROR;
    int iTotalLen = iSqlLen + 128 + 64*columnMap.size() + table_ptuser.ByteSizeLong();
    int iLen = 7;
    GORM_MallocFromSharedPool(pMemPool, pReqData, iTotalLen);
    szSQLBegin = pReqData->m_uszData;
    memcpy(szSQLBegin, "select ", 7);
    int idx = 0;
    vector<string> &vectorColumns = pMySQLEvent->m_mapTablesColumnOrder["ptuser"];
    if (vectorColumns.size() != columnMap.size())
        return GORM_ERROR;
    for (auto c : vectorColumns)
    {
    ...
    const int32 ptuser_pttype = table_ptuser.pttype();
    iLen += GORM_SafeSnprintf(szSQLBegin+iLen, iTotalLen-iLen, PTUSERGETSQL_DEBUG_WHERE, iTableIndex, sz_ptuser_ptid, ptuser_pttype);
    pReqData->m_sUsedSize = iLen;
...
#ifdef GORM_DEBUG
    GORM_MySQLUpdateTableSchema(pMySQLEvent, "ptuser", table.custom_columns());
    return GORM_PackGetSQLPTUSER_ONE_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table_ptuser, pReqData);
#endif
...
int GORM_PackGetSQLUSER_ONE_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_Table_user &table_user, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
    GORM_MySQLUpdateTableSchema(pMySQLEvent, "user", table.custom_columns());
    return GORM_PackGetSQLUSER_ONE_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table_user, pReqData);
#endif
...
int GORM_PackUpdateSQLROLE(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, MYSQL* mysql, int iTableIndex, const GORM_PB_UPDATE_REQ* pMsg, GORM_MemPoolData *&pReqData)
{
    ...
    int iWhereLen = iSqlLen + 128  + len_role_rolename + len_role_face + 104;
    ...
    int iLen = iSqlLen + 128 + pMsg->ByteSizeLong() + len_role_rolename+ len_role_face+104;
    GORM_MallocFromSharedPool(pMemPool, pReqData, iLen+iWhereLen+1);
    szSQLBegin = pReqData->m_uszData;
...
int GORM_PackIncreaseSQLROLE(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, MYSQL* mysql, int iTableIndex, const GORM_PB_INCREASE_REQ* pMsg, GORM_MemPoolData *&pReqData)
{
    ...
    int iLen = iSqlLen + 128 + pMsg->ByteSizeLong() +136;
    GORM_MallocFromSharedPool(pMemPool, pReqData, iLen+iWhereLen+1);
    szSQLBegin = pReqData->m_uszData;
...
iLen *= 2;
    GORM_MallocFromSharedPool(pMemPool, pReqData, iLen<<1);
    szSQLBegin = pReqData->m_uszData;
    iLen = GORM_SafeSnprintf(szSQLBegin, iLen, ROLEREPLACESQL, iTableIndex, role_version, role_roleid, sz_role_rolename, role_level, role_exp, role_characterid, role_gold, role_offlinetime, role_inited, role_createtime, sz_role_face, role_online, role_worldid, role_pttype, role_userid, sz_role_rolename,role_level,role_exp,role_characterid,role_gold,role_offlinetime,role_inited,role_createtime, sz_role_face,role_online,role_worldid,role_pttype,role_userid);
    pReqData->m_sUsedSize = iLen;
...
int GORM_PackGet_By_Non_Primary_KeySQLEQUIP_One_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_CUSTEM_COLUMNS &pbColumns, const GORM_PB_Table_equip &table_equip, const GORM_PB_REQ_HEADER &header, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
        GORM_MySQLUpdateTableSchema(pMySQLEvent, "equip", table.custom_columns());
        return GORM_PackGet_By_Non_Primary_KeySQLEQUIP_One_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table.custom_columns(), table_equip, pMsg->header(), pReqData);
#endif
...
int GORM_PackGet_By_Non_Primary_KeySQLFOOD_One_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_CUSTEM_COLUMNS &pbColumns, const GORM_PB_Table_food &table_food, const GORM_PB_REQ_HEADER &header, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
        GORM_MySQLUpdateTableSchema(pMySQLEvent, "food", table.custom_columns());
        return GORM_PackGet_By_Non_Primary_KeySQLFOOD_One_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table.custom_columns(), table_food, pMsg->header(), pReqData);
#endif
        return GORM_PackGet_By_Non_Primary_KeySQLFOOD_One(pMemPool, mysql, iTableIndex, table_food, pMsg->header(), pReqData);
    }
    return GORM_OK;
}
#define GetByNonPrimaySQL_HERO "select `version`,`roleid`,`snakeid`,`heros`,`teams` from hero_%d where "
...
#ifdef GORM_DEBUG
#define GetByNonPrimaySQL_HERO_DEBUG "select `version`,`roleid`,`snakeid`,`heros`,`teams`"
int GORM_PackGet_By_Non_Primary_KeySQLHERO_One_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_CUSTEM_COLUMNS &pbColumns, const GORM_PB_Table_hero &table_hero, const GORM_PB_REQ_HEADER &header, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
        GORM_MySQLUpdateTableSchema(pMySQLEvent, "hero", table.custom_columns());
        return GORM_PackGet_By_Non_Primary_KeySQLHERO_One_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table.custom_columns(), table_hero, pMsg->header(), pReqData);
#endif
        return GORM_PackGet_By_Non_Primary_KeySQLHERO_One(pMemPool, mysql, iTableIndex, table_hero, pMsg->header(), pReqData);
    }
    return GORM_OK;
}
#define GetByNonPrimaySQL_LOGIN_LOG "select `version`,`roleid`,`action`,`online`,`ip`,`appid`,`createtime` from login_log_%d where "
int GORM_PackGet_By_Non_Primary_KeySQLLOGIN_LOG_One(shared_ptr<GORM_MemPool> &pMemPool, MYSQL* mysql, int iTableIndex, const GORM_PB_Table_login_log &table_login_log, const GORM_PB_REQ_HEADER &header, GORM_MemPoolData *&pReqData)
{
    ...
    int iTotalLen = 64*vFields.size() + iLen + table_login_log.ByteSizeLong();
    GORM_MallocFromSharedPool(pMemPool, pReqData, iTotalLen);
    char *szSQLBegin = pReqData->m_uszData;
...
#ifdef GORM_DEBUG
#define GetByNonPrimaySQL_LOGIN_LOG_DEBUG "select `version`,`roleid`,`action`,`online`,`ip`,`appid`,`createtime`"
int GORM_PackGet_By_Non_Primary_KeySQLLOGIN_LOG_One_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_CUSTEM_COLUMNS &pbColumns, const GORM_PB_Table_login_log &table_login_log, const GORM_PB_REQ_HEADER &header, GORM_MemPoolData *&pReqData)
{
    ...
    if (fieldMode == "")
        return GORM_REQ_NO_RECORDS;
    vector<int> vFields = GORM_FieldsOpt::GetFields(fieldMode.c_str(), fieldMode.size());
    int iLen = strlen(GetByNonPrimaySQL_LOGIN_LOG_DEBUG);
    int iTotalLen = 64*vFields.size() + pbColumns.ByteSizeLong() + iLen + table_login_log.ByteSizeLong();
    GORM_MallocFromSharedPool(pMemPool, pReqData, iTotalLen);
    char *szSQLBegin = pReqData->m_uszData;
    strncpy(szSQLBegin, GetByNonPrimaySQL_LOGIN_LOG_DEBUG, iLen);
    vector<string> &vColumns = pMySQLEvent->m_mapTablesColumnOrder["login_log"];
    for (int i=7; i<vColumns.size(); i++)
    {
        iLen += GORM_SafeSnprintf(szSQLBegin+iLen, iTotalLen-iLen, ", `%s`", vColumns[i].c_str());
    }
    iLen += GORM_SafeSnprintf(szSQLBegin+iLen, iTotalLen-iLen, " from login_log where ");
    for(int i=0; i<vFields.size(); i++)
    {
        switch (vFields[i])
        {
...
#ifdef GORM_DEBUG
        GORM_MySQLUpdateTableSchema(pMySQLEvent, "login_log", table.custom_columns());
        return GORM_PackGet_By_Non_Primary_KeySQLLOGIN_LOG_One_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table.custom_columns(), table_login_log, pMsg->header(), pReqData);
#endif
...
int GORM_PackGet_By_Non_Primary_KeySQLMAIL_One_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_CUSTEM_COLUMNS &pbColumns, const GORM_PB_Table_mail &table_mail, const GORM_PB_REQ_HEADER &header, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
        GORM_MySQLUpdateTableSchema(pMySQLEvent, "mail", table.custom_columns());
        return GORM_PackGet_By_Non_Primary_KeySQLMAIL_One_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table.custom_columns(), table_mail, pMsg->header(), pReqData);
#endif
...
int GORM_PackGet_By_Non_Primary_KeySQLMATERIAL_One_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_CUSTEM_COLUMNS &pbColumns, const GORM_PB_Table_material &table_material, const GORM_PB_REQ_HEADER &header, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
        GORM_MySQLUpdateTableSchema(pMySQLEvent, "material", table.custom_columns());
        return GORM_PackGet_By_Non_Primary_KeySQLMATERIAL_One_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table.custom_columns(), table_material, pMsg->header(), pReqData);
#endif
...
int GORM_PackGet_By_Non_Primary_KeySQLNPC_One_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_CUSTEM_COLUMNS &pbColumns, const GORM_PB_Table_npc &table_npc, const GORM_PB_REQ_HEADER &header, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
        GORM_MySQLUpdateTableSchema(pMySQLEvent, "npc", table.custom_columns());
        return GORM_PackGet_By_Non_Primary_KeySQLNPC_One_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table.custom_columns(), table_npc, pMsg->header(), pReqData);
#endif
        return GORM_PackGet_By_Non_Primary_KeySQLNPC_One(pMemPool, mysql, iTableIndex, table_npc, pMsg->header(), pReqData);
    }
    return GORM_OK;
}
#define GetByNonPrimaySQL_RES_LOG "select `version`,`roleid`,`action`,`cfgtype`,`cfgid`,`delta`,`value`,`createtime` from res_log_%d where "
int GORM_PackGet_By_Non_Primary_KeySQLRES_LOG_One(shared_ptr<GORM_MemPool> &pMemPool, MYSQL* mysql, int iTableIndex, const GORM_PB_Table_res_log &table_res_log, const GORM_PB_REQ_HEADER &header, GORM_MemPoolData *&pReqData)
{
    ...
    int iTotalLen = 64*vFields.size() + iLen + table_res_log.ByteSizeLong();
    GORM_MallocFromSharedPool(pMemPool, pReqData, iTotalLen);
    char *szSQLBegin = pReqData->m_uszData;
...
#ifdef GORM_DEBUG
#define GetByNonPrimaySQL_RES_LOG_DEBUG "select `version`,`roleid`,`action`,`cfgtype`,`cfgid`,`delta`,`value`,`createtime`"
int GORM_PackGet_By_Non_Primary_KeySQLRES_LOG_One_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_CUSTEM_COLUMNS &pbColumns, const GORM_PB_Table_res_log &table_res_log, const GORM_PB_REQ_HEADER &header, GORM_MemPoolData *&pReqData)
{
    ...
    if (fieldMode == "")
        return GORM_REQ_NO_RECORDS;
    vector<int> vFields = GORM_FieldsOpt::GetFields(fieldMode.c_str(), fieldMode.size());
    int iLen = strlen(GetByNonPrimaySQL_RES_LOG_DEBUG);
    int iTotalLen = 64*vFields.size() + pbColumns.ByteSizeLong() + iLen + table_res_log.ByteSizeLong();
    GORM_MallocFromSharedPool(pMemPool, pReqData, iTotalLen);
    char *szSQLBegin = pReqData->m_uszData;
    strncpy(szSQLBegin, GetByNonPrimaySQL_RES_LOG_DEBUG, iLen);
    vector<string> &vColumns = pMySQLEvent->m_mapTablesColumnOrder["res_log"];
    for (int i=8; i<vColumns.size(); i++)
    {
        iLen += GORM_SafeSnprintf(szSQLBegin+iLen, iTotalLen-iLen, ", `%s`", vColumns[i].c_str());
    }
    iLen += GORM_SafeSnprintf(szSQLBegin+iLen, iTotalLen-iLen, " from res_log where ");
    for(int i=0; i<vFields.size(); i++)
    {
        switch (vFields[i])
        {
...
#ifdef GORM_DEBUG
        GORM_MySQLUpdateTableSchema(pMySQLEvent, "res_log", table.custom_columns());
        return GORM_PackGet_By_Non_Primary_KeySQLRES_LOG_One_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table.custom_columns(), table_res_log, pMsg->header(), pReqData);
#endif
...
int GORM_PackGet_By_Non_Primary_KeySQLINTERACTION_One_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_CUSTEM_COLUMNS &pbColumns, const GORM_PB_Table_interaction &table_interaction, const GORM_PB_REQ_HEADER &header, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
        GORM_MySQLUpdateTableSchema(pMySQLEvent, "interaction", table.custom_columns());
        return GORM_PackGet_By_Non_Primary_KeySQLINTERACTION_One_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table.custom_columns(), table_interaction, pMsg->header(), pReqData);
#endif
...
int GORM_PackGet_By_Non_Primary_KeySQLROLES_One_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_CUSTEM_COLUMNS &pbColumns, const GORM_PB_Table_roles &table_roles, const GORM_PB_REQ_HEADER &header, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
        GORM_MySQLUpdateTableSchema(pMySQLEvent, "roles", table.custom_columns());
        return GORM_PackGet_By_Non_Primary_KeySQLROLES_One_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table.custom_columns(), table_roles, pMsg->header(), pReqData);
#endif
        return GORM_PackGet_By_Non_Primary_KeySQLROLES_One(pMemPool, mysql, iTableIndex, table_roles, pMsg->header(), pReqData);
    }
    return GORM_OK;
}
#define GetByNonPrimaySQL_ROLE "select `version`,`roleid`,`rolename`,`level`,`exp`,`characterid`,`gold`,`offlinetime`,`inited`,`createtime`,`face`,`online`,`worldid`,`pttype`,`userid` from role_%d where "
...
#ifdef GORM_DEBUG
#define GetByNonPrimaySQL_ROLE_DEBUG "select `version`,`roleid`,`rolename`,`level`,`exp`,`characterid`,`gold`,`offlinetime`,`inited`,`createtime`,`face`,`online`,`worldid`,`pttype`,`userid`"
int GORM_PackGet_By_Non_Primary_KeySQLROLE_One_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_CUSTEM_COLUMNS &pbColumns, const GORM_PB_Table_role &table_role, const GORM_PB_REQ_HEADER &header, GORM_MemPoolData *&pReqData)
{
    ...
    for (int i=15; i<vColumns.size(); i++)
    {
        iLen += GORM_SafeSnprintf(szSQLBegin+iLen, iTotalLen-iLen, ", `%s`", vColumns[i].c_str());
    }
...
#ifdef GORM_DEBUG
        GORM_MySQLUpdateTableSchema(pMySQLEvent, "role", table.custom_columns());
        return GORM_PackGet_By_Non_Primary_KeySQLROLE_One_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table.custom_columns(), table_role, pMsg->header(), pReqData);
#endif
...
int GORM_PackGet_By_Non_Primary_KeySQLSCENE_One_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_CUSTEM_COLUMNS &pbColumns, const GORM_PB_Table_scene &table_scene, const GORM_PB_REQ_HEADER &header, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
        GORM_MySQLUpdateTableSchema(pMySQLEvent, "scene", table.custom_columns());
        return GORM_PackGet_By_Non_Primary_KeySQLSCENE_One_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table.custom_columns(), table_scene, pMsg->header(), pReqData);
#endif
...
int GORM_PackGet_By_Non_Primary_KeySQLSKILL_One_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_CUSTEM_COLUMNS &pbColumns, const GORM_PB_Table_skill &table_skill, const GORM_PB_REQ_HEADER &header, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
        GORM_MySQLUpdateTableSchema(pMySQLEvent, "skill", table.custom_columns());
        return GORM_PackGet_By_Non_Primary_KeySQLSKILL_One_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table.custom_columns(), table_skill, pMsg->header(), pReqData);
#endif
        return GORM_PackGet_By_Non_Primary_KeySQLSKILL_One(pMemPool, mysql, iTableIndex, table_skill, pMsg->header(), pReqData);
    }
    return GORM_OK;
}
#define GetByNonPrimaySQL_PTUSER "select `version`,`ptid`,`pttype`,`userid` from ptuser_%d where "
int GORM_PackGet_By_Non_Primary_KeySQLPTUSER_One(shared_ptr<GORM_MemPool> &pMemPool, MYSQL* mysql, int iTableIndex, const GORM_PB_Table_ptuser &table_ptuser, const GORM_PB_REQ_HEADER &header, GORM_MemPoolData *&pReqData)
{
    ...
    int iTotalLen = 64*vFields.size() + iLen + table_ptuser.ByteSizeLong();
    GORM_MallocFromSharedPool(pMemPool, pReqData, iTotalLen);
    char *szSQLBegin = pReqData->m_uszData;
...
#ifdef GORM_DEBUG
#define GetByNonPrimaySQL_PTUSER_DEBUG "select `version`,`ptid`,`pttype`,`userid`"
int GORM_PackGet_By_Non_Primary_KeySQLPTUSER_One_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_CUSTEM_COLUMNS &pbColumns, const GORM_PB_Table_ptuser &table_ptuser, const GORM_PB_REQ_HEADER &header, GORM_MemPoolData *&pReqData)
{
    ...
    if (fieldMode == "")
        return GORM_REQ_NO_RECORDS;
    vector<int> vFields = GORM_FieldsOpt::GetFields(fieldMode.c_str(), fieldMode.size());
    int iLen = strlen(GetByNonPrimaySQL_PTUSER_DEBUG);
    int iTotalLen = 64*vFields.size() + pbColumns.ByteSizeLong() + iLen + table_ptuser.ByteSizeLong();
    GORM_MallocFromSharedPool(pMemPool, pReqData, iTotalLen);
    char *szSQLBegin = pReqData->m_uszData;
    strncpy(szSQLBegin, GetByNonPrimaySQL_PTUSER_DEBUG, iLen);
    vector<string> &vColumns = pMySQLEvent->m_mapTablesColumnOrder["ptuser"];
    for (int i=4; i<vColumns.size(); i++)
    {
        iLen += GORM_SafeSnprintf(szSQLBegin+iLen, iTotalLen-iLen, ", `%s`", vColumns[i].c_str());
    }
    iLen += GORM_SafeSnprintf(szSQLBegin+iLen, iTotalLen-iLen, " from ptuser where ");
    for(int i=0; i<vFields.size(); i++)
    {
        switch (vFields[i])
        {
...
#ifdef GORM_DEBUG
        GORM_MySQLUpdateTableSchema(pMySQLEvent, "ptuser", table.custom_columns());
        return GORM_PackGet_By_Non_Primary_KeySQLPTUSER_One_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table.custom_columns(), table_ptuser, pMsg->header(), pReqData);
#endif
        return GORM_PackGet_By_Non_Primary_KeySQLPTUSER_One(pMemPool, mysql, iTableIndex, table_ptuser, pMsg->header(), pReqData);
    }
    return GORM_OK;
}
#define GetByNonPrimaySQL_USER "select `version`,`userid`,`ptid`,`pttype`,`createtime`,`rolesdata` from user_%d where "
...
#ifdef GORM_DEBUG
#define GetByNonPrimaySQL_USER_DEBUG "select `version`,`userid`,`ptid`,`pttype`,`createtime`,`rolesdata`"
int GORM_PackGet_By_Non_Primary_KeySQLUSER_One_DEBUG(shared_ptr<GORM_MemPool> &pMemPool, GORM_MySQLEvent *pMySQLEvent, int iTableIndex, const GORM_PB_CUSTEM_COLUMNS &pbColumns, const GORM_PB_Table_user &table_user, const GORM_PB_REQ_HEADER &header, GORM_MemPoolData *&pReqData)
{
...
#ifdef GORM_DEBUG
        GORM_MySQLUpdateTableSchema(pMySQLEvent, "user", table.custom_columns());
        return GORM_PackGet_By_Non_Primary_KeySQLUSER_One_DEBUG(pMemPool, pMySQLEvent, iTableIndex, table.custom_columns(), table_user, pMsg->header(), pReqData);
#endif
...
#endif
#ifdef GORM_DEBUG
int GORM_MySQLResult2PbMSG_LOGIN_LOG_DEBUG(GORM_MySQLEvent *pMySQLEvent, GORM_PB_Table_login_log *pPbTable, GORM_PB_CUSTEM_COLUMNS *pColumns, MYSQL_ROW row, unsigned long *lengths)
{
    if (GORM_OK != GORM_MySQLResult2PbMSG_LOGIN_LOG(pPbTable, row, lengths))
        return GORM_ERROR;
    return GORM_OK;
    vector<string> &vTablesColumnOrder = pMySQLEvent->m_mapTablesColumnOrder["login_log"];
    unordered_map<string, GORM_PB_COLUMN_TYPE> &mapTablesColumnType = pMySQLEvent->m_mapTablesColumnInfo["login_log"];
    for (int i=7; i<vTablesColumnOrder.size(); i++)
    {
        GORM_PB_COLUMN *pColumn = pColumns->add_columns();
        string &strColumn = vTablesColumnOrder[i];
...
#endif
#ifdef GORM_DEBUG
int GORM_MySQLResult2PbMSG_RES_LOG_DEBUG(GORM_MySQLEvent *pMySQLEvent, GORM_PB_Table_res_log *pPbTable, GORM_PB_CUSTEM_COLUMNS *pColumns, MYSQL_ROW row, unsigned long *lengths)
{
    if (GORM_OK != GORM_MySQLResult2PbMSG_RES_LOG(pPbTable, row, lengths))
        return GORM_ERROR;
    return GORM_OK;
    vector<string> &vTablesColumnOrder = pMySQLEvent->m_mapTablesColumnOrder["res_log"];
    unordered_map<string, GORM_PB_COLUMN_TYPE> &mapTablesColumnType = pMySQLEvent->m_mapTablesColumnInfo["res_log"];
    for (int i=8; i<vTablesColumnOrder.size(); i++)
    {
        GORM_PB_COLUMN *pColumn = pColumns->add_columns();
        string &strColumn = vTablesColumnOrder[i];
        pColumn->set_name(strColumn);
...
int GORM_MySQLResult2PbMSG_ROLE_DEBUG(GORM_MySQLEvent *pMySQLEvent, GORM_PB_Table_role *pPbTable, GORM_PB_CUSTEM_COLUMNS *pColumns, MYSQL_ROW row, unsigned long *lengths)
{
    ...
    for (int i=15; i<vTablesColumnOrder.size(); i++)
    {
        GORM_PB_COLUMN *pColumn = pColumns->add_columns();
        string &strColumn = vTablesColumnOrder[i];
        pColumn->set_name(strColumn);
...
#endif
#ifdef GORM_DEBUG
int GORM_MySQLResult2PbMSG_PTUSER_DEBUG(GORM_MySQLEvent *pMySQLEvent, GORM_PB_Table_ptuser *pPbTable, GORM_PB_CUSTEM_COLUMNS *pColumns, MYSQL_ROW row, unsigned long *lengths)
{
    if (GORM_OK != GORM_MySQLResult2PbMSG_PTUSER(pPbTable, row, lengths))
        return GORM_ERROR;
    return GORM_OK;
    vector<string> &vTablesColumnOrder = pMySQLEvent->m_mapTablesColumnOrder["ptuser"];
    unordered_map<string, GORM_PB_COLUMN_TYPE> &mapTablesColumnType = pMySQLEvent->m_mapTablesColumnInfo["ptuser"];
    for (int i=4; i<vTablesColumnOrder.size(); i++)
    {
        GORM_PB_COLUMN *pColumn = pColumns->add_columns();
        string &strColumn = vTablesColumnOrder[i];
...
int GORM_MySQLResult2PbMSG_USER_DEBUG(GORM_MySQLEvent *pMySQLEvent, GORM_PB_Table_user *pPbTable, GORM_PB_CUSTEM_COLUMNS *pColumns, MYSQL_ROW row, unsigned long *lengths)
{
    ...
    for (int i=6; i<vTablesColumnOrder.size(); i++)
    {
        GORM_PB_COLUMN *pColumn = pColumns->add_columns();
        string &strColumn = vTablesColumnOrder[i];
        pColumn->set_name(strColumn);
...
int GORM_MySQLResult2PbMSG(GORM_MySQLEvent *pMySQLEvent, int iTableId, GORM_PB_TABLE *pPbTable, MYSQL_ROW row, unsigned long *lengths)
{
    ...
    case GORM_PB_TABLE_IDX_LOGIN_LOG:
    {
        ...
        GORM_PB_CUSTEM_COLUMNS *pColumns = pPbTable->mutable_custom_columns();
        return GORM_MySQLResult2PbMSG_LOGIN_LOG_DEBUG(pMySQLEvent, pSrcTable, pColumns, row, lengths);
    #else
        GORM_PB_Table_login_log *pSrcTable = pPbTable->mutable_login_log();
    ...
    case GORM_PB_TABLE_IDX_RES_LOG:
    {
        ...
        GORM_PB_CUSTEM_COLUMNS *pColumns = pPbTable->mutable_custom_columns();
        return GORM_MySQLResult2PbMSG_RES_LOG_DEBUG(pMySQLEvent, pSrcTable, pColumns, row, lengths);
    #else
        GORM_PB_Table_res_log *pSrcTable = pPbTable->mutable_res_log();
    ...
    case GORM_PB_TABLE_IDX_PTUSER:
    {
        ...
        GORM_PB_CUSTEM_COLUMNS *pColumns = pPbTable->mutable_custom_columns();
        return GORM_MySQLResult2PbMSG_PTUSER_DEBUG(pMySQLEvent, pSrcTable, pColumns, row, lengths);
    #else
        GORM_PB_Table_ptuser *pSrcTable = pPbTable->mutable_ptuser();
