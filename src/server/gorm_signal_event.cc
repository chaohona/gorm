...
GORM_SignalEvent::GORM_TransferEvent(shared_ptr<GORM_Epoll>       pEpoll, GORM_FrontEndThread *pThread):
    GORM_Event(0, pEpoll), m_pFrontThread(pThread)
{
    m_iDataFlag = 0;
}

GORM_SignalEvent::~GORM_TransferEvent()
{
    if (this->m_iReadFD != 0)
...
int GORM_SignalEvent::Init()
{
    ...
    this->m_iReadFD = pipe[0];
    this->m_iWriteFD = pipe[1];
...
void GORM_SignalEvent::Send(bool bForce)
{
    // 已经有数据了，则不再往里面写数据了
    if (!bForce)
    {
