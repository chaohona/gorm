...
func Get() bool {
	request, ret := gormSvr.GetRequestForGet("role")
	...
	if ret := record.SetFieldIntValue("roleid", 107); ret != define.GORM_CODE_OK {
...
func GetOrInsert() bool {
	request, ret := gormSvr.GetRequestForGetOrInsert("role")
	if request == nil || ret != define.GORM_CODE_OK {
		fmt.Println("get insert request for account failed.")
		return false
	}
	...
	if ret := record.SetFieldIntValue("roleid", 10000); ret != define.GORM_CODE_OK {
		fmt.Println("set value for record failed, code:%d", ret)
		return false
	}
	...
	if ret := record.SetFieldIntValue("exp", 10000); ret != define.GORM_CODE_OK {
		fmt.Println("set value for record failed, code:%d", ret)
		return false
	}
