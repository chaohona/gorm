...
int TestGetByNonPrimaryKey()
{
    GORM_ClientRequest *pRequest = g_global_server.GetRequestForGetByNonPrimaryKey("account");
    if (pRequest == nullptr)
    {
        std::cout << "get request failed" << std::endl;
        return -1;
    ...
    // 设置record
    if (GORM_OK != pRecord->SetFieldValue("id", 1))
    {
        std::cout << "set record value failed, for id" << std::endl;
        return -1;
    }
