...
int TestUpdate() 
{
    ...
    // 将新的结果带回来
    pRequest->SetResultFlag(GORM_ResultFlag_RETURN_ALL_VALUE);
...
int TestGetByPartKey()
{
    ...
    // 设置record
    if (GORM_OK != pRecord->SetFieldValue("id", 200))
    {
        std::cout << "set record value failed, for id" << std::endl;
        return -1;
    }
    if (GORM_OK != pRecord->SetFieldValue("account", "200_account"))
    {
        std::cout << "set record value failed, for account" << std::endl;
        return -1;
    }
...
int main()
{
    ...
    // 由于设置了回带新的数据，所以可以在响应结果中获取操作新的数据
    if (0 != TestReplace())
    {
        return -1;
    }
    ...
    for(;;)
    {
        ...
        if (pResponse != nullptr)
        {
            ...
            /////////如果对UPDATE消息格式比较了解，可以直接操作update响应消息pGetPbMsg，这样效率最高///////////////////////////
            gorm::GORM_PB_UPDATE_RSP *pGetPbMsg = dynamic_cast<GORM_PB_UPDATE_RSP*>(pPbMsg);

            // 通过接口获取数据
            GORM_Record *pResultRecord = nullptr;
            do
            {
                iRet = pResponse->GetNextRecord(pResultRecord);
				if (iRet == GORM_NO_MORE_RECORD)
					break;
                ...
                std::string strAccountDescField;
                pResultRecord->GetFieldValue(gorm::GORM_PB_ACCOUNT_FIELD_INDEX::GORM_PB_FIELD_ACCOUNT_DESC, strAccountDescField);
                std::cout << "got result from api, id:" << iAccountID << ", account field:" << strAccountAccField << ", desc field:" << strAccountDescField << std::endl;
